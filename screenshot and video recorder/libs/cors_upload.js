...
var MediaUploader = function (t) {
  ...
  if (
    ((this.file = t.file),
    (this.contentType =
      t.contentType || this.file.type || "application/octet-stream"),
    ...
    (n.uploadType = "resumable"),
      (this.url = this.buildUrl_(t.fileId, n, t.baseUrl));
  }
...
(MediaUploader.prototype.upload = function () {
  ...
  (MediaUploader.prototype.sendFile_ = function () {
    ...
    (this.offset || this.chunkSize) &&
      (this.chunkSize &&
        (e = Math.min(this.offset + this.chunkSize, this.file.size)),
      (t = t.slice(this.offset, e)));
    var n = new XMLHttpRequest();
    n.open("PUT", this.url, !0),
      n.setRequestHeader("Content-Type", this.contentType),
      n.setRequestHeader(
        "Content-Range",
        "bytes " + this.offset + "-" + (e - 1) + "/" + this.file.size
      ),
      n.setRequestHeader("X-Upload-Content-Type", this.file.type),
      n.upload && n.upload.addEventListener("progress", this.onProgress),
      (n.onload = this.onContentUploadSuccess_.bind(this)),
      (n.onerror = this.onContentUploadError_.bind(this)),
      n.send(t);
  }),
  ...
  (MediaUploader.prototype.onContentUploadSuccess_ = function (t) {
    200 == t.target.status || 201 == t.target.status
      ...
      : 308 == t.target.status &&
        (this.extractRange_(t.target),
        this.retryHandler.reset(),
        this.sendFile_());
  }),
  (MediaUploader.prototype.onContentUploadError_ = function (t) {
    t.target.status && t.target.status < 500
